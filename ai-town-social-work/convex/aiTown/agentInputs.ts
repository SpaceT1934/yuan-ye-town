...
export const agentInputs = {
  removeAgent: inputHandler({
    ...
    handler: (game, now, args) => {
      const parsedPlayerId = parseGameId('players', args.playerId);
      const player = game.world.players.get(parsedPlayerId);
      if (!player) return null;
      const agent = [...game.world.agents.values()].find(
        (candidate) => candidate.playerId === parsedPlayerId,
      );
      player.leave(game, now);
      if (agent) game.world.agents.delete(agent.id);
      return null;
    },
  }),
