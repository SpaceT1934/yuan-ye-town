  ...
  handler: async (ctx, { now, worldId, player, otherFreePlayers }) => {
    ...
    for (const otherPlayer of otherFreePlayers) {
      ...
      const [player1, player2] =
        player.id.localeCompare(otherPlayer.id) < 0
      ...
      const relationship = await ctx.db
        ...
        .unique();
      const socialScore = relationship
        ? relationship.conflict * 0.18 - relationship.affection * 0.08 - relationship.trust * 0.05
        : 0;
      candidates.push({ id: otherPlayer.id, position: otherPlayer.position, socialScore });
    }

    // Prefer nearby residents, while established positive relationships make another
    // conversation more likely and conflict makes avoidance more likely.
    candidates.sort(
      (a, b) =>
        distance(a.position, position) +
        a.socialScore -
        (distance(b.position, position) + b.socialScore),
    );
