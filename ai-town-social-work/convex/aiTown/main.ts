...
import { rebasePausedWorld } from './resumeClock';
...
export async function startEngine(ctx: MutationCtx, worldId: Id<'worlds'>) {
  ...
  await rebasePausedWorld(ctx, worldId, engine.currentTime ?? now, now);
...
export async function kickEngine(ctx: MutationCtx, worldId: Id<'worlds'>) {
  ...
  const generationNumber = engine.generationNumber + 1;
  const now = Date.now();
  const stale = now - (engine.currentTime ?? now) > ENGINE_ACTION_DURATION * 2;
  if (stale) await rebasePausedWorld(ctx, worldId, engine.currentTime ?? now, now);
  await ctx.db.patch(engineId, {
    generationNumber,
    ...(stale ? { currentTime: now, lastStepTs: now } : {}),
  });
  await ctx.scheduler.runAfter(0, internal.aiTown.main.runStep, {
    worldId: worldId,
    generationNumber,
