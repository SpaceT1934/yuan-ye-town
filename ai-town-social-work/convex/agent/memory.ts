...
export async function rememberConversation(
  ...
  const llmMessages: LLMMessage[] = [
    {
      role: 'user',
      content: `你是${player.name}，刚刚结束了与${otherPlayer.name}的对话。请只用简体中文，
      从${player.name}的第一人称视角总结这次交谈，使用“我”来叙述，并说明你喜欢还是不喜欢这次互动。`,
    },
  ];
  ...
  ).toLocaleString('zh-CN')}与${otherPlayer.name}的对话：${content}`;
...
async function calculateImportance(description: string) {
  const { content: importanceRaw } = await chatCompletion({
    messages: [
      {
        role: 'user',
        content: `请评估下面这段记忆的重要程度。0代表非常平常（例如刷牙、整理床铺），9代表极其重要（例如分手、收到大学录取通知）。
      记忆：${description}
      只回复一个0到9之间的数字，例如“5”。`,
      },
    ],
...
async function reflectOnMemories(
  ...
  const prompt = ['[不要输出解释]', '[只输出合法 JSON]', `你是${name}，下面是与你有关的陈述：`];
  memories.forEach((m, idx) => {
    prompt.push(`陈述 ${idx}：${m.description}`);
  });
  prompt.push('请从以上陈述中推断出3条更高层次的中文洞察。');
  prompt.push(
    '只返回可由 Typescript JSON.parse() 直接解析的 JSON 数组。每项使用 insight 保存简体中文洞察，statementIds 保存支持该洞察的陈述编号。不要使用 Markdown 代码块。',
  );
  prompt.push(
