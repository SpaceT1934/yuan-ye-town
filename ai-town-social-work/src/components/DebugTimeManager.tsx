...
export function DebugTimeManager(props: {
  ...
  if (intervals.length > 0) {
    ...
    intervalNode = (
      <div style={{ fontSize: '12px' }}>
        {intervals.length} 个时间区间：
        <div style={{ fontSize: '9px', height: '48px' }}>
          <p>基准：{toSeconds(baseAge)} 秒前</p>
          {intervals.map((interval) => {
            ...
            let serverTs = null;
            if (containsServerTs) {
              serverTs = `（服务器：${toSeconds((timeManager.prevServerTs ?? base) - base)}）`;
            }
            return (
              <div
  ...
  if (timeManager.latestEngineStatus) {
    ...
    const statusMsg = status.running ? '运行中' : '已停止';
    statusNode = (
      <div style={{ fontSize: '12px', paddingTop: '8px' }}>
        <p>世代编号：{status.generationNumber}</p>
        <p>输入编号：{status.processedInputNumber}</p>
        <p>状态：{statusMsg}</p>
        <p>客户端时钟偏差：{toSeconds(timeManager.clockSkew())} 秒</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div style={{ height: '20px', width: '100%', textAlign: 'center' }}>引擎状态</div>
      {statusNode}
